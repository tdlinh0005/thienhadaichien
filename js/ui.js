...
U.m_hamdoi = function () {
  ...
  if (st.toi.length) {
    h += '<div class="panel"><h3>Hạm đội địch đang bay tới</h3><div class="noi bang-cuon"><table>' +
      '<tr><th>Chỉ huy</th><th>Từ</th><th>Đánh vào</th><th>Còn</th></tr>';
    for (i = 0; i < st.toi.length; i++) {
      var w = st.toi[i], pt = st.planets[w.pi] || st.planets[0];
      h += '<tr><td>' + U.esc(w.ten) + ' <span class="tag-lm">' + U.esc(w.lm || '') + '</span></td><td class="sz">' +
        G.tdStr(w.tu) + '</td><td>' + U.esc(pt.ten) + ' ' + G.tdStr(pt.c) + '</td><td class="sz do">' + U.dem(w.den_t) + '</td></tr>';
    }
    h += '</table><p class="mo">Hạm đội đang bay thì không bị bắn hạ — nếu không đỡ được, hãy cho hạm đội bay đi trước khi địch tới.</p></div></div>';
  }
