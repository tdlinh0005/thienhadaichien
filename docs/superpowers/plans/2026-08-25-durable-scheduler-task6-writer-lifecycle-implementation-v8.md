# Durable Scheduler Task 6 Writer Lifecycle Implementation Plan V8
...
V8 was written after direct inspection of live source:

- `node --test --test-isolation=none --test-reporter=tap` on live Node `v24.19.0` emits named TAP subtests.
...
- `server/scheduler/store.js`: live claim APIs are `claimNext(token,nowMs,watermarkS,lockMs)`, which claims only ACCOUNT_ADVANCE, and `claimForResolution(token,jobId,nowMs,lockMs,options)`, which claims one chosen global/logical row. Live canonical helpers inside Store include `validateJob`, `canonicalJson`, `sha256`, `parseCanonicalBoundedJson`, `executableInput`, `mutationConflict`, `requiredMutation`, and `fail`.
- `replaceAccountAdvance(token,accountId,revision,nextLocalAtS,nowMs)` uses idempotency key `account-advance:<accountId>:<revision>`. If that key already exists with a different scheduled time/payload, Store schedule would throw `IDEMPOTENCY_PAYLOAD_MISMATCH`; therefore direct external-only same-key retarget cannot be implemented with `replaceAccountAdvance` alone.
...
- Preserve V1-V7 plan files. This planning task creates only this V8 file.
...
- The active V8 plan hash is not embedded in this plan. After this file is frozen, implementation preflight must run `sha256sum docs/superpowers/plans/2026-08-25-durable-scheduler-task6-writer-lifecycle-implementation-v8.md > /tmp/task6-active-plan.sha256` and finish with `sha256sum -c /tmp/task6-active-plan.sha256`.

## V8 correction ledger

1. Adds Task6-owned Store primitive `retargetOwnedPendingAccountAdvanceForCommand(token,accountId,revision,targetS,nowMs)` for same-key direct external-only retargets. It preserves job id, idempotency key, sequence, attempt, and max attempts; regenerates canonical `payload_json`/`payload_sha256`; updates `scheduled_at_s` and payload `nextLocalAtS`; rejects RUNNING, blocked, terminal, wrong account/revision, and lease-loss states.
2. Direct external-only command finalization now tries retarget first for the same-key World-created PENDING wake. If no same-key wake exists, it calls `replaceAccountAdvance` once to schedule dependency `T`, then adopts and blocks. If an existing blocked row is already installed, writer returns that blocked row without double-blocking.
3. Restores explicit parent global-first drain/admission path: `claimFirstBarrierForDrain`, `settleBarriersForAdmission`, and account `claimNext` only after global barrier logic. Global jobs are never omitted.
4. Every claim UoW, both global and account, calls `recordEffectiveNowInCurrentUow` before claim and uses that returned effective time for locks. Case 13 includes a backward wall-clock restart after claim.
5. Test manifest remains exactly 42 tests by consolidating retarget primitive subcases into case 40 and folding global-first claim into cases 13/19/33.
6. Normalized inventory allowlist includes the exact `docs/superpowers/reports` directory and exact report file because the report directory may not exist before implementation.
7. All V1-V7 fixes remain binding: exact Task5 downstream, already-parked global barrier, zero-global metric, strict partial effect, fatal settlement transitions, raw quarantine with no payload parse, private seams, one standby poll, exact CLI, external plan hash, report without self-hash, and two fresh Sol/high reviews over exact six implementation/test hashes.
...
- [ ] **Step 1: Pin active V8 plan externally**

Run before any source/test edit:

```bash
sha256sum docs/superpowers/plans/2026-08-25-durable-scheduler-task6-writer-lifecycle-implementation-v8.md \
...
var TASK6_RED_CASES = Object.freeze([
  ['Task 6 v8 writer rejects legacy mode before lease', 'TASK6V8_RED_001_LEGACY'],
  ['Task 6 v8 startup acquisition rollback arms no poll while stopping', 'TASK6V8_RED_002_STARTUP_ROLLBACK'],
  ['Task 6 v8 startup committed lease release preserves original failure', 'TASK6V8_RED_003_STARTUP_RELEASE'],
  ['Task 6 v8 startup recovery marks expired rows asserts integrity stages resume and fences', 'TASK6V8_RED_004_STARTUP_RECOVERY'],
  ['Task 6 v8 heartbeat transient rearms and generation loss keeps one poll', 'TASK6V8_RED_005_HEARTBEAT'],
  ['Task 6 v8 lease loss during stop finalization is idempotent', 'TASK6V8_RED_006_LEASE_STOP'],
  ['Task 6 v8 shutdown admission fence drains captured tail with bounded grace', 'TASK6V8_RED_007_SHUTDOWN'],
  ['Task 6 v8 private lifecycle seams are non enumerable and db close cache only', 'TASK6V8_RED_008_PRIVATE'],
  ['Task 6 v8 runMaintenanceCutover fresh only lease audit and release', 'TASK6V8_RED_009_CUTOVER'],
  ['Task 6 v8 scheduler cutover CLI exports exact surface and closes kho', 'TASK6V8_RED_010_CLI'],
  ['Task 6 v8 claim brands raw row before target default', 'TASK6V8_RED_011_BRAND'],
  ['Task 6 v8 before claim crash leaves no running row', 'TASK6V8_RED_012_BEFORE_CLAIM'],
  ['Task 6 v8 after claim crash uses effective lock time and recovers backward clock restart', 'TASK6V8_RED_013_AFTER_CLAIM_CLOCK'],
  ['Task 6 v8 after application crash restarts without duplicate effect', 'TASK6V8_RED_014_AFTER_APP'],
  ['Task 6 v8 after effect crash rolls back application and world state', 'TASK6V8_RED_015_AFTER_EFFECT'],
  ['Task 6 v8 before final fence generation flip rolls back effect UoW', 'TASK6V8_RED_016_FINAL_FENCE'],
  ['Task 6 v8 already applied restart skips effect and terminalizes once', 'TASK6V8_RED_017_REPLAY'],
  ['Task 6 v8 stale generation effect fails before commit', 'TASK6V8_RED_018_STALE_GENERATION'],
  ['Task 6 v8 global first drain claims barrier before account and parks once', 'TASK6V8_RED_019_GLOBAL_FIRST'],
  ['Task 6 v8 zero budget global synthesizes exact advance metric', 'TASK6V8_RED_020_ZERO_GLOBAL'],
  ['Task 6 v8 account partial effect shape and identity are exact', 'TASK6V8_RED_021_PARTIAL_SHAPE'],
  ['Task 6 v8 account partial blocked external uses advanceResult dependency', 'TASK6V8_RED_022_PARTIAL_BLOCK'],
  ['Task 6 v8 established zero partial retains running lock unchanged', 'TASK6V8_RED_023_ESTABLISHED_ZERO'],
  ['Task 6 v8 prepared global charges primitive once before idempotency insert', 'TASK6V8_RED_024_GLOBAL_CHARGE'],
  ['Task 6 v8 prepared replay validates application and skips second effect', 'TASK6V8_RED_025_REPLAY_CHARGE'],
  ['Task 6 v8 terminal mapping uses only accepted Store methods', 'TASK6V8_RED_026_TERMINAL'],
  ['Task 6 v8 account cancelled terminal is not deferred and returns afterCancel', 'TASK6V8_RED_027_ACCOUNT_CANCEL'],
  ['Task 6 v8 deferred completed account records advance once before finalizer', 'TASK6V8_RED_028_DEFER_METRIC'],
  ['Task 6 v8 settlement reloads executable then Store fail', 'TASK6V8_RED_029_SETTLE_FAIL'],
  ['Task 6 v8 settlement raw corruption quarantines without payload parse', 'TASK6V8_RED_030_RAW_QUARANTINE'],
  ['Task 6 v8 settlement missing fresh row preserves original failure', 'TASK6V8_RED_031_MISSING_ROW'],
  ['Task 6 v8 settlement short circuits injected lease and fatal originals', 'TASK6V8_RED_032_SETTLE_SHORT'],
  ['Task 6 v8 runCommand barrier partial returns TICK_PARTIAL', 'TASK6V8_RED_033_CMD_BARRIER'],
  ['Task 6 v8 runCommand account partial skips closure and queues continuation', 'TASK6V8_RED_034_CMD_PARTIAL'],
  ['Task 6 v8 runCommand thenable rolls back after invocation only', 'TASK6V8_RED_035_CMD_THENABLE'],
  ['Task 6 v8 account delete skips advance and adopted success returns closure', 'TASK6V8_RED_036_CMD_DELETE_ADOPT'],
  ['Task 6 v8 advance due 61 account boundary skips closure and queues continuation', 'TASK6V8_RED_037_CMD_DUE_61'],
  ['Task 6 v8 direct completed account preserves World synchronized successor', 'TASK6V8_RED_038_DIRECT_COMPLETE'],
  ['Task 6 v8 direct established zero creates exact fallback continuation', 'TASK6V8_RED_039_DIRECT_ZERO'],
  ['Task 6 v8 retargets direct external only same key wake then adopts and blocks', 'TASK6V8_RED_040_DIRECT_RETARGET'],
  ['Task 6 v8 RED manifest and normalized scope catch report dir symlink dirs and extra files', 'TASK6V8_RED_041_SCOPE_MANIFEST'],
  ['Task 6 v8 implementation seal excludes report and active plan is fixed', 'TASK6V8_RED_042_SEAL']
]);
```

Task assignments are exact: 1-8 lifecycle/private seams, 9-10 cutover/CLI, 11-32 claim/downstream/store/settlement, 33-40 command/direct-account behavior, 41-42 scope/seal. Case 40 contains sub-assertions for real World save, same-key retarget, idempotent repeat, wrong state, lease loss, and later wake retention; it is one TAP subtest and one manifest case.
...
- [ ] **Step 2: Add Store retarget primitive for direct same-key wake**
...
SchedulerStore.prototype.retargetOwnedPendingAccountAdvanceForCommand = function (
  ...
  var normalized = validateJob({kind: 'ACCOUNT_ADVANCE', scheduledAtS: targetS,
    ...
    sourceAccountId: null, maxAttempts: 8, replayOfJobId: null,
...
Primitive tests are inside case 40:

- real World save creates a same-key PENDING wake for the receipt revision;
- same-key wake is retargeted to dependency `T` without changing `id`, `sequence`, `idempotency_key`, `attempt`, or `max_attempts`;
- exact repeat with same `targetS` is idempotent and returns the same row;
- RUNNING, blocked PENDING, terminal rows, wrong account/revision, and lost lease fail with exact `ACCOUNT_ADVANCE_RETARGET_CONFLICT` or `LEASE_LOST`;
- existing later wake becomes deterministic retained history only when fallback scheduling needs `replaceAccountAdvance`; same-key retarget does not create an extra wake.
...
- [ ] **Step 5: Implement drain loop and admission global barrier settlement**
...
SchedulerWriter.prototype.drainNow = function () {
  ...
  return this.enqueue(async function () {
    ...
    try {
      while (budget.value > 0) {
        ...
        if (!claimed) {
          ...
          claimed = self.takeNextAccountJob(token, nowMs, watermarkS);
        }
        if (!claimed) return;
        var outcome;
        try {
          ...
          self.settleClaimFailure(claimed, error);
          self.observeJobMetric(claimed, 'error', Math.max(0, self.effectiveNowMs() - nowMs));
          continue;
        }
    ...
    if (self.ready) self.scheduleWake();
  });
};
...
SchedulerWriter.prototype.settleBarriersForAdmission = function (budget, nowS, nowMs) {
  ...
  while (true) {
    ...
    try {
      ...
      this.settleClaimFailure(claimed, error);
      this.observeJobMetric(claimed, 'error', Math.max(0, this.effectiveNowMs() - nowMs));
      return {deferred: true, processed: startingBudget - budget.value};
    }
    ...
    if (outcome.budgetExhausted) return Object.assign({}, outcome, {
      processed: startingBudget - budget.value
    });
  }
};
...
`runCommand` must call `settleBarriersForAdmission` before marker/closure writes. If it returns partial/deferred, return `{deferred:true,code:'TICK_PARTIAL'}`, enqueue after commit, and do not invoke closure.
...
Add Store primitive `quarantineClaimedRaw(token,rawRow,failure,nowMs)` with no payload parse. It uses only safe raw id/lock fields, sanitized failure code/message, live lease SQL fence, state `QUARANTINED`, null lock fields, null retry, and does not touch `attempt`, `payload_json`, `payload_sha256`, `max_attempts`, or replay fields.
...
- [ ] **Step 4: Implement direct external-only retarget/adopt/block**
...
- direct command uses real `world.advanceAccountNoiBo` and real `world.luu` to create same-key wake for `receipt.revision`;
- `retargetOwnedPendingAccountAdvanceForCommand` retargets that same job id to dependency `T`;
- second retarget with identical args is idempotent;
- RUNNING, blocked, terminal, wrong account/revision, and lost lease paths fail with exact codes;
- when no same-key wake exists, writer calls `replaceAccountAdvance` exactly once, then adopts and blocks;
- an existing later unblocked wake is deterministically superseded only by the `replaceAccountAdvance` fallback, and the blocked continuation later schedules the next successor through `completeAccountAdvanceAndScheduleSuccessor`.
...
- [ ] **Step 7: Implement `runCommand` with global-first admission and honest thenable semantics**
...
The report includes these six hashes, verification outputs, RED proof summary, inventory helper hash, scope checks, and the fact that review binding is to the six-file seal plus external V8 plan hash only. The report does not include its own hash.
...
expected_fixed = {
  'docs/superpowers/plans/2026-08-25-durable-scheduler-task6-writer-lifecycle-implementation-v8.md',
}
def load(path):
  rows = {}
...
This detects non-owned tracked, untracked, mode-only, type, directory, file, symlink target, deletion, and arbitrary untracked additions while allowing only the exact report directory and report file.

- [ ] **Step 5: Launch two fresh Sol/high reviews over exact seals**

Send reviewers:

- V8 plan path and external hash from `/tmp/task6-active-plan.sha256`,
...
Any change to the six sealed files or V8 plan invalidates both approvals and requires rerunning verification, recomputing six hashes, and launching two new fresh Sol/high reviews.

- [ ] **Step 6: Finalize report and rerun final checks**

Approvals may be recorded in the report after review because review binding is to the six implementation/test hashes, inventory helper hash, and external V8 plan hash. If report text changes after approval, rerun:
...
- Direct external-only same-key retarget uses `retargetOwnedPendingAccountAdvanceForCommand`, not `replaceAccountAdvance`, when World already created the same idempotency key at a different time.
- Retarget primitive regenerates canonical payload and hash with live Store helpers and preserves id/key/attempt.
- If no same-key wake exists, writer calls `replaceAccountAdvance` once, then adopts and blocks.
- Global-first drain uses `claimFirstBarrierForDrain`/`claimForResolution` before account `claimNext`.
- Every claim UoW records effective now before claim and uses it for locks.
- Manifest is exactly 42 unique `TASK6V8_RED_*` sentinels, with cases 1-42 assigned as stated.
- Normalized inventory allows exact report directory plus report file and no other non-owned changes.
- Plan has no self-hash requirement; external `/tmp/task6-active-plan.sha256` is authoritative.

## Completion definition

- V1-V7 plans are preserved.
- V8 active plan hash is pinned externally and remains fixed through final handoff.
...
- Store retarget primitive handles direct external-only same-key wake retargets atomically and lease-fenced.
...
- Direct account commands use normal World save options, preserve direct completed successor, handle established-zero fallback, retarget dependency-safe external-only fallback wakes, and never commit a missing continuation.
...
- Scope checks prove no non-owned tracked, untracked, mode-only, type, directory, file, or symlink change except exact report directory/report.
- Report has no self-hash. Two fresh Sol/high approvals bind to exact six implementation/test hashes, external V8 plan hash, and inventory helper hash.
