# Durable Scheduler Task 6 Writer Lifecycle Implementation Plan V20
...
**Architecture:** V20 keeps the simplified count from V19: parent runtime `73` plus overlay runtime `7`, giving `N=80`; accepted baseline `163/162/0/1` becomes final scheduler TAP `243/242/0/1`. V20 fixes the parent import gap by importing and sealing helper-only JS prerequisites before the parent73 block, then appending overlays after parent helpers. Overlay tests prove only new Task6 seams not already frozen by accepted Task5 tests or parent73; source review verifies integration wiring for contracts intentionally tested directly at Store level.
...
- Preserve V1-V19 plan artifacts. This planning task creates only this V20 file.
...
- Dropped V18 replay and partial-wrapper overlays remain absent. Those protocols are accepted by Task5 baseline tests and parent73.
- Parent Task6, accepted Task5, and live source APIs override any V1-V19 conflict.
- No report self-hash and no embedded V20 self-hash; root passes `TASK6_V20_APPROVED_SHA` externally.
...
1. Capture approved V20 hash, authority hashes, PRE inventory, baseline TAP/names, and prefix bytes before any source/test/report edit.
...
3. Append V20 helper-only prerequisite JS between `// TASK6_V20_HELPERS_START` and `// TASK6_V20_HELPERS_END`.
4. Append faithful parent Task6 runtime block between `// TASK6_V20_PARENT_TESTS_START` and `// TASK6_V20_PARENT_TESTS_END`.
5. Verify parent-only registration loads exactly 73 runtime names and does not register prerequisite tests.
6. Append V20 overlay7 between `// TASK6_V20_OVERLAY_TESTS_START` and `// TASK6_V20_OVERLAY_TESTS_END`.
7. Extract expected helper, parent, and overlay JS from pinned artifacts/approved plan and `cmp` them byte-for-byte with current test slices.
8. Run isolated overlay RED exactly once. The RED harness rejects `ReferenceError`, missing prerequisite, fixture setup failures, and `MODULE_NOT_FOUND`; each failure must contain the case sentinel from the targeted first method/export assertion.
9. Implement Store/writer/index/cutover/CLI in TDD waves.
10. Run Stage-A validation, launch two fresh Sol/high implementation reviews over exact six implementation/test hashes plus V20 plan/evidence artifacts, append genuine approvals to the report only after both PASS, then run Stage-B.

## Parent helper prerequisite import

The parent73 block depends on helper functions defined by earlier parent Task4/5 sections. V20 imports helper-only JS before parent73; no Task4/5 test registrations are copied.

Required source ranges from pinned parent plan:

- Base world helpers: lines `6411-6470` from the `6407` JS fence: `giayTuClock`, `taoMutationTam`, `taoDongHoS`, `themTaiKhoan`, `docState`, `attackTo`, `taoWorldSchedulerTam`.
- Task5 imports/helper: lines `7593-7609`, adapted to avoid duplicate declarations and to provide `GameAdvanceService`, `toPublicAdvanceResult`, `EventReducer`, `resolveCanonicalGlobalInCurrentUow`.
- PvP fixture helpers: lines `7677-7744`: `coDinhTimeline`, `holdAt`, `luuQuaMutation`, `taoPvpFixtureAt`.
- Fixture execution helpers: lines `7748-7850`: `executeFixtureClaimedInCurrentUow`, `damBaoLeaseFixture`, `tuaFixtureQuaDichVu`, `apDungPrepared`, `giaiPvp`.
- Account/reopen helpers: lines `7855-7904`: `taoAccountAdvanceFixture`, `moLaiPvpFixture`.
- Local event helpers: raw lines `8134-8144`: `holdArrivingAt`, `datNoiTaiCungGiay`. This range is inside an already-open JS fence, so it is copied as raw JS lines rather than parsed by a local opening fence.

Exact V20 helper import block inserted before extracted helper bodies:
...
Do not redeclare `Kho`, `TheGioi`, `assert`, `test`, `fs`, `path`, `G`, `fakeClock`, `NOW_MS`, `taoKhoTam`, `dongKhoTam`, `removeDb`, `taoStoreTam`, `layLease`, or `job`; those already exist in the accepted live test prelude.

Static helper free-identifier allowlist for V20:
...
holdArrivingAt
holdAt
installAdvanceServiceFixture
job
layLease
luuQuaMutation
path
resolveCanonicalGlobalInCurrentUow
taoAccountAdvanceFixture
taoDongHoS
taoKhoTam
taoMutationTam
taoPvpFixtureAt
taoStoreTam
taoWorldSchedulerTam
themTaiKhoan
toPublicAdvanceResult
```

Any additional unbound identifier found in helper or parent slices is a plan execution failure unless it is declared by the accepted live prelude before the V20 marker.

## Production contracts retained and corrected

### Downstream Task5 protocol

Every executable calls `reducer.prepare(mutation, executable, options)`, including committed replay. The writer must follow accepted Task5 ordering:
...
Required Task6 additions in `server/scheduler/store.js`:

- `quarantineClaimedRaw(token, rawRow, failure, nowMs) -> {id,state:'QUARANTINED',errorCode}`:
  - Never parse `rawRow.payload_json`; overlay A uses invalid JSON bytes to prove this.
...
- `retargetOwnedPendingAccountAdvanceForCommand(token, accountId, revision, targetS, nowMs) -> descriptor|null`:
  - Validate same-key PENDING `ACCOUNT_ADVANCE` by canonical payload/hash first; persisted identity/canonical contradiction maps `PAYLOAD_INTEGRITY`.
  ...
  - Return only frozen descriptor `{id, accountId, revision, scheduledAtS}`.
...
Required Task6 behavior in `server/scheduler/writer.js`:
...
- Implement `computeOpenStatusFromStore(writer)`, `canonicalSchedulerStatus(status)`, and `cloneSchedulerStatus(status)` inside `writer.js`; no metrics module dependency in Task6. Validate raw values/keys before normalization. Clone nested `jobAttempts`, `jobDuration`, `leaseAcquire`, and `reconcile`. Public status includes flattened counts/ages, `counts`, `ages`, lifecycle booleans, `continuationActive`, metrics, and never `effectiveNowMs`. Use `max(lastEffectiveNowMs, clock.nowMs())` for read ages.
- Expired held lease synchronously transitions lease-lost, clears mutation/retry timers, retains exactly one standby poll. `_datDatabaseClosing()` snapshots safe public status before `dbOpen=false`; after close, `status()` and bridge `getStatus()` are cache-only and return `ready:false`, `dbOpen:false`, `reason:'SCHEDULER_DB_CLOSED'` without SQLite.
- Private Task7 seams are non-enumerable functions: `_datSignalHandlerInstalled(installed)`, `_beginStop(reason, graceMs)`, `_waitForStopFinalization()`, `_datDatabaseClosing()`.
- Timer cleanup: `clearMutationTimers()`, `clearPollTimer()`, `clearGraceTimer()`, and `clearAllTimers()`. Fatal/crashed/beginStop/stop/DB-close call `clearAllTimers()`. Lease loss alone calls `clearMutationTimers()` then ensures exactly one standby acquisition poll.
- Startup: validate mode/options, acquire lease, preview/list expired and owned running rows, marker before `recoverExpiredRunning` when preview has expired rows, recover expired rows, call `assertAccountDependencyIntegrity(token,effective)`, renew lease with fresh effective time after long startup work, rebase surviving owned RUNNING locks with `resumeOwnedRunning(token,id,freshNow,leaseMs)`, final lease/row fence before commit, then publish staged partial ids/timers after commit. Writer must not set `ready=true` until after the fresh renew/final fence UoW commits.
- Claim/drain: every claim UoW calls `recordEffectiveNowInCurrentUow` before listing/claiming. `takeNextJob` checks owned RUNNING global resume first, then global barrier classification, due RETRY_WAIT, quarantine rules, foreign RUNNING, watermark gating, and only then account work. `partialQueue` contains string ids and rotates watermark-ineligible partials.
...
- `advanceDueInCurrentUow(mutation, nowS, budget)` asserts `world._schedulerActive(mutation)`, queries `kho.q.dqDenHan.all(nowS, 61)`, processes first 60 in returned order only, and on a 61st row returns command partial, persists `TICK_PARTIAL`, enqueues continuation after commit, and never invokes the command closure. Do not edit `server/world.js`.
- `runCommand({name, accountId?, run})` derives effective monotonic time internally. It drains due work before invoking the closure. Any global/account partial or 61-boundary due partial returns defined `{deferred:true, code:'TICK_PARTIAL'}` and schedules continuation after commit; `manualDrain` never arms implicit continuation.
- Direct external continuation: normal World save must not pass `deferAccountWake:true`. When direct account result has `deferredExternal`/`blockedExternalJobId`, retarget same-key wake first; if absent, create once with `replaceAccountAdvance`; validate any existing or returned row with `validateBlockedAccountAdvanceForDependency`. Only finalizer records metrics.
...
### Index, cutover, and CLI

- `server/scheduler/index.js` exports exactly `{taoScheduler, inRange, resolveDurableSchedulerOptions}`. Bridge `getStatus` calls `writer.status()`.
- `runMaintenanceCutover({kho, clock, ownerId})` validates context/owner before Store construction where possible, runs migration before Store, acquires a 15,000ms fresh lease, rejects non-legacy mode with `SCHEDULER_CUTOVER_ALREADY_DURABLE`, rejects any existing `dq` row with `SCHEDULER_CUTOVER_RECONCILIATION_REQUIRED`, seeds only `combat_seed_key_v1`, writes durable mode and `CUTOVER` audit, and releases the same token in `finally`. If both primary and release fail, rethrow primary; if only release fails, throw release.
- `tools/scheduler-cutover.js` exports exactly `{parseArgs, runCli, main}`. `parseArgs(argv)` accepts exactly `['--db', nonemptyFile, '--action', 'cutover']` in that order and rejects duplicate/unknown/missing/extra tokens, empty path, or directory before opening. `runCli(argv,deps)` validates before open, derives owner using `assertSchedulerOwnerId((deps.makeOwnerId || crypto.randomUUID)())`, defaults clock to live `taoClock()`, closes via `kho.dong()` in `finally`, returns `{exitCode, stdout, stderr}`, and applies primary-vs-close precedence. `main` writes returned streams, sets `process.exitCode`, returns the result, and never calls `process.exit`.

## Standalone artifact creation

Approved plan, authorities, and TAP normalizer:

```bash
set -euo pipefail
test -n "$TASK6_V20_APPROVED_SHA"
case "$TASK6_V20_APPROVED_SHA" in (*[!0-9a-f]*|'') exit 1 ;; esac
test "${#TASK6_V20_APPROVED_SHA}" -eq 64
printf '%s  docs/superpowers/plans/2026-08-25-durable-scheduler-task6-writer-lifecycle-implementation-v20.md\n' \
  "$TASK6_V20_APPROVED_SHA" > /tmp/task6-v20-approved-plan.sha256
sha256sum -c /tmp/task6-v20-approved-plan.sha256
printf '%s  docs/superpowers/plans/2026-08-23-durable-event-scheduler-implementation.md\n' \
  cbc05d4b8127f7f530bb13789f6fce0aee961e576ced279871d0f43ec67cb8d3 > /tmp/task6-v20-authorities.sha256
printf '%s  docs/superpowers/plans/2026-08-24-durable-scheduler-task5-remediation-implementation.md\n' \
  c89ba74ce9431ef479e91ffc996e0d9f4adde4217a157756c50ce4bf49ec3eaf >> /tmp/task6-v20-authorities.sha256
sha256sum -c /tmp/task6-v20-authorities.sha256
cat > /tmp/task6-v20-tap-normalize.sh <<'SH'
normalize_tap_for_hash() {
  sed -E '/^[[:space:]]+duration_ms:/d;/^# duration_ms /d' "$1"
}
SH
sha256sum /tmp/task6-v20-tap-normalize.sh > /tmp/task6-v20-tap-normalize.sh.sha256
```

Inventory helper and PRE inventory. The normalized repo inventory deliberately excludes the exact report file and exact reports directory entry from content inventory/diff to avoid report self-hash cycles; it does not exclude any other file under `docs/superpowers/reports`.

```bash
cat > /tmp/task6-v20-inventory.sh <<'SH'
repo_inventory() {
  python3 - <<'PY'
...
SH
sha256sum /tmp/task6-v20-inventory.sh > /tmp/task6-v20-inventory.sh.sha256
. /tmp/task6-v20-inventory.sh
repo_inventory > /tmp/task6-v20-pre-inventory.tsv
sha256sum /tmp/task6-v20-pre-inventory.tsv > /tmp/task6-v20-pre-inventory.tsv.sha256
```

Baseline TAP/names and prefix bytes:

```bash
node --test --test-isolation=none --test-reporter=tap tools/test-scheduler.js > /tmp/task6-v20-baseline.tap
grep -E '^# tests 163$' /tmp/task6-v20-baseline.tap
grep -E '^# pass 162$' /tmp/task6-v20-baseline.tap
grep -E '^# fail 0$' /tmp/task6-v20-baseline.tap
grep -E '^# skipped 1$' /tmp/task6-v20-baseline.tap
node - <<'NODE' /tmp/task6-v20-baseline.tap > /tmp/task6-v20-baseline-names.txt
const fs = require('fs');
...
NODE
sha256sum /tmp/task6-v20-baseline-names.txt > /tmp/task6-v20-baseline-names.txt.sha256
node - <<'NODE' > /tmp/task6-v20-prefix.tsv
const fs = require('fs'), crypto = require('crypto');
const data = fs.readFileSync('tools/test-scheduler.js');
for (const marker of [
  '// TASK6_V20_HELPERS_START',
  '// TASK6_V20_HELPERS_END',
  '// TASK6_V20_PARENT_TESTS_START',
  '// TASK6_V20_PARENT_TESTS_END',
  '// TASK6_V20_OVERLAY_TESTS_START',
  '// TASK6_V20_OVERLAY_TESTS_END'
]) {
  if (data.includes(Buffer.from(marker))) throw new Error('TASK6_V20_ALREADY_PRESENT');
}
console.log([data.length, crypto.createHash('sha256').update(data).digest('hex')].join('\t'));
NODE
sha256sum /tmp/task6-v20-prefix.tsv > /tmp/task6-v20-prefix.tsv.sha256
```

Helper-only expected slice:

```bash
cat > /tmp/task6-v20-helper-imports.js <<'EOF'
var {
  GameAdvanceService,
  toPublicAdvanceResult
} = require('../server/scheduler/advance-service.js');
...
sed -n '6411,6470p' "$parent" > /tmp/task6-v20-helper-base.js
sed -n '7601,7609p' "$parent" > /tmp/task6-v20-helper-advance.js
sed -n '7677,7744p' "$parent" > /tmp/task6-v20-helper-pvp.js
sed -n '7748,7850p' "$parent" > /tmp/task6-v20-helper-exec.js
sed -n '7855,7904p' "$parent" > /tmp/task6-v20-helper-account.js
sed -n '8134,8144p' "$parent" > /tmp/task6-v20-helper-local.js
cat /tmp/task6-v20-helper-imports.js \
  /tmp/task6-v20-helper-base.js \
  /tmp/task6-v20-helper-advance.js \
  /tmp/task6-v20-helper-pvp.js \
  /tmp/task6-v20-helper-exec.js \
  /tmp/task6-v20-helper-account.js \
  /tmp/task6-v20-helper-local.js > /tmp/task6-v20-expected-helper-slice.js
test -s /tmp/task6-v20-expected-helper-slice.js
if rg -n '^[[:space:]]*test[[:space:]]*\\(' /tmp/task6-v20-expected-helper-slice.js; then exit 1; fi
sha256sum /tmp/task6-v20-expected-helper-slice.js > /tmp/task6-v20-expected-helper-slice.js.sha256
sha256sum /tmp/task6-v20-helper-*.js > /tmp/task6-v20-helper-parts.sha256
cat > /tmp/task6-v20-helper-free-identifiers.txt <<'EOF'
assert
EventReducer
G
GameAdvanceService
...
toPublicAdvanceResult
EOF
sha256sum /tmp/task6-v20-helper-free-identifiers.txt > /tmp/task6-v20-helper-free-identifiers.txt.sha256
```

Parent73 expected names and source slice:

```bash
cat > /tmp/task6-v20-parent73-names.txt <<'EOF'
...
EOF
wc -l /tmp/task6-v20-parent73-names.txt | grep -E '^73 '
sha256sum /tmp/task6-v20-parent73-names.txt > /tmp/task6-v20-parent73-names.txt.sha256
sed -n '10886,11844p' "$parent" > /tmp/task6-v20-parent-block-1.md
sed -n '11848,12485p' "$parent" > /tmp/task6-v20-parent-block-2.md
sed -n '12489,12833p' "$parent" > /tmp/task6-v20-parent-block-3.md
sed -n '12837,13070p' "$parent" > /tmp/task6-v20-parent-block-4.md
sha256sum /tmp/task6-v20-parent-block-*.md > /tmp/task6-v20-parent-blocks.sha256
python3 - <<'PY' /tmp/task6-v20-parent-block-1.md /tmp/task6-v20-parent-block-2.md /tmp/task6-v20-parent-block-3.md /tmp/task6-v20-parent-block-4.md > /tmp/task6-v20-expected-parent-slice.js
import pathlib, sys
...
PY
test -s /tmp/task6-v20-expected-parent-slice.js
sha256sum /tmp/task6-v20-expected-parent-slice.js > /tmp/task6-v20-expected-parent-slice.js.sha256
```

## V20 overlay JS authority and body

Stage extracts expected JS from the approved V20 plan, not from the current test file:

```bash
awk '/^\/\/ TASK6_V20_PLAN_OVERLAY_JS_START$/{flag=1; next} /^\/\/ TASK6_V20_PLAN_OVERLAY_JS_END$/{flag=0} flag' \
  docs/superpowers/plans/2026-08-25-durable-scheduler-task6-writer-lifecycle-implementation-v20.md \
  > /tmp/task6-v20-expected-overlay.js
test -s /tmp/task6-v20-expected-overlay.js
sha256sum /tmp/task6-v20-expected-overlay.js > /tmp/task6-v20-expected-overlay.js.sha256
```

```js
// TASK6_V20_PLAN_OVERLAY_JS_START
function task6v20Sentinel(sentinel, body) {
...
function task6v20AssertMethod(object, name, sentinel) {
  assert.equal(typeof object[name], 'function', sentinel + ' missing ' + name);
}
function task6v20Rows(kho, sql, params) {
  var stmt = kho.db.prepare(sql);
  return stmt.all.apply(stmt, params || []);
}

test('Task 6 v20 overlay raw quarantine store primitive preserves attempt without parsing invalid payload',
  task6v20Sentinel('TASK6V20_RED_001_RAW_QUARANTINE_STORE', async function () {
    var x = taoWriterFixture();
    try {
      task6v20AssertMethod(x.store, 'quarantineClaimedRaw', 'TASK6V20_RED_001_RAW_QUARANTINE_STORE');
...
test('Task 6 v20 overlay status cache private seams and exact scheduler exports',
  task6v20Sentinel('TASK6V20_RED_002_STATUS_EXPORTS', async function () {
    ...
    try {
      ...
      assert.equal(calls, 1, 'TASK6V20_RED_002_STATUS_EXPORTS bridge calls writer.status');
      ...
      assert.equal(x.writer.metrics.jobDuration.ACCOUNT_ADVANCE, undefined);
      assert.notEqual(x.writer.metrics.leaseAcquire.acquired, 99);
      assert.notEqual(x.writer.metrics.reconcile.success, 99);
      ['_datSignalHandlerInstalled', '_beginStop', '_waitForStopFinalization', '_datDatabaseClosing']
        .forEach(function (name) {
          task6v20AssertMethod(x.writer, name, 'TASK6V20_RED_002_STATUS_EXPORTS');
          assert.equal(Object.keys(x.writer).includes(name), false);
        });
      ...
      await x.writer._beginStop('test-stop', 100);
      await x.writer._waitForStopFinalization();
    } finally { await x.writer.stop(100).catch(function () {}); x.dong(); }
  }));

test('Task 6 v20 overlay retarget and blocked descriptor store primitives use dependency source account',
  task6v20Sentinel('TASK6V20_RED_003_RETARGET_DESCRIPTOR', async function () {
    var T = 1_800_020_000;
    var x = taoPvpFixtureAt(T);
    try {
      task6v20AssertMethod(x.store, 'retargetOwnedPendingAccountAdvanceForCommand', 'TASK6V20_RED_003_RETARGET_DESCRIPTOR');
      task6v20AssertMethod(x.store, 'validateBlockedAccountAdvanceForDependency', 'TASK6V20_RED_003_RETARGET_DESCRIPTOR');
      x.clock.setS(T);
...
test('Task 6 v20 overlay startup renews after recovery before ready and publishes after commit',
  task6v20Sentinel('TASK6V20_RED_004_STARTUP_RENEW', async function () {
    var x;
    ...
    try {
      ['listExpiredRunningForRecovery', 'listOwnedRunningForRecovery', 'resumeOwnedRunning']
        .forEach(function (name) { task6v20AssertMethod(x.store, name, 'TASK6V20_RED_004_STARTUP_RENEW'); });
      task6v20AssertMethod(x.writer, 'refreshLeasePhaseInCurrentUow', 'TASK6V20_RED_004_STARTUP_RENEW');
      ...
      x.store.renewLease = function (token, nowMs, leaseMs) {
        ...
        assert.equal(x.writer.ready, false, 'TASK6V20_RED_004_STARTUP_RENEW renew occurs before ready');
        assert.equal(timers.soDangCho(), 0, 'TASK6V20_RED_004_STARTUP_RENEW no timer before commit');
        assert.ok(nowMs >= entryNow + 14_900, 'TASK6V20_RED_004_STARTUP_RENEW renew uses fresh post-recovery time');
      ...
      await x.writer.start();
      assert.ok(order.indexOf('listExpired') >= 0);
      assert.ok(order.indexOf('listOwned') >= 0);
      assert.ok(order.indexOf('recover') > order.indexOf('listExpired'));
      assert.ok(order.some(function (item) { return item.indexOf('renew:') === 0; }));
      assert.ok(finalFenceCalls >= 1);
      assert.ok(resumeCalls >= 0);
      assert.equal(x.writer.status().ready, true);
      assert.ok(timers.soDangCho() > 0, 'TASK6V20_RED_004_STARTUP_RENEW timers publish after ready commit');
...
test('Task 6 v20 overlay advance due keeps live order and enqueues continuation at sixty one',
  task6v20Sentinel('TASK6V20_RED_005_DUE61_ORDER', async function () {
    var x = taoWriterFixture({manualDrain: false});
    var timers = taoBoHenGia(x.clock);
    var processed = [], closureRan = false, helperCalls = 0;
    try {
      task6v20AssertMethod(x.writer, 'advanceDueInCurrentUow', 'TASK6V20_RED_005_DUE61_ORDER');
...
test('Task 6 v20 overlay maintenance cutover validates guards and release precedence',
  task6v20Sentinel('TASK6V20_RED_006_CUTOVER_RELEASE', async function () {
    ...
    task6v20AssertMethod(cutover, 'runMaintenanceCutover', 'TASK6V20_RED_006_CUTOVER_RELEASE');
    ...
    try {
      ...
      try {
        ...
        ).run('v20-cutover-legacy', 'V20 Cutover Legacy', 'h', 's', 1, 1).lastInsertRowid);
...
test('Task 6 v20 overlay scheduler cutover CLI grammar lifecycle and exact exports',
  task6v20Sentinel('TASK6V20_RED_007_CLI', async function () {
    ...
    try {
      ...
      try {
        var legacyId = Number(primaryClose.kho.db.prepare(
          'INSERT INTO tk(ten,hienthi,mk,muoi,tao,vaoCuoi) VALUES(?,?,?,?,?,?)'
        ).run('v20-cli-legacy', 'V20 CLI Legacy', 'h', 's', 1, 1).lastInsertRowid);
        ...
        assert.equal(primaryWins.exitCode, 1);
...
```bash
set -euo pipefail
cat > /tmp/task6-v20-overlay7-names.txt <<'EOF'
Task 6 v20 overlay raw quarantine store primitive preserves attempt without parsing invalid payload
Task 6 v20 overlay status cache private seams and exact scheduler exports
Task 6 v20 overlay retarget and blocked descriptor store primitives use dependency source account
Task 6 v20 overlay startup renews after recovery before ready and publishes after commit
Task 6 v20 overlay advance due keeps live order and enqueues continuation at sixty one
Task 6 v20 overlay maintenance cutover validates guards and release precedence
Task 6 v20 overlay scheduler cutover CLI grammar lifecycle and exact exports
EOF
sha256sum /tmp/task6-v20-overlay7-names.txt > /tmp/task6-v20-overlay7-names.txt.sha256
test ! -e /tmp/task6-v20-red-evidence.tap
: > /tmp/task6-v20-red-evidence.tap
run_one_overlay_red() {
  set -euo pipefail
  name="$1"; sentinel="$2"
...
NODE
  ...
  tmp="$(mktemp /tmp/task6-v20-overlay-red.XXXXXX.tap)"
  ...
  grep -F "$sentinel" "$tmp" >/dev/null || { cat "$tmp"; rm -f "$tmp"; exit 1; }
  if grep -E 'MODULE_NOT_FOUND|ReferenceError|MISSING_PREREQUISITE|is not defined|fixture setup|fixture job must' "$tmp"; then
    cat "$tmp"; rm -f "$tmp"; exit 1
  fi
  printf '%s\nname=%s\nsentinel=%s\n' '### TASK6_V20_OVERLAY_RED_CASE_START' "$name" "$sentinel" >> /tmp/task6-v20-red-evidence.tap
  cat "$tmp" >> /tmp/task6-v20-red-evidence.tap
  printf '%s\n' '### TASK6_V20_OVERLAY_RED_CASE_END' >> /tmp/task6-v20-red-evidence.tap
  rm -f "$tmp"
}
run_one_overlay_red 'Task 6 v20 overlay raw quarantine store primitive preserves attempt without parsing invalid payload' 'TASK6V20_RED_001_RAW_QUARANTINE_STORE'
run_one_overlay_red 'Task 6 v20 overlay status cache private seams and exact scheduler exports' 'TASK6V20_RED_002_STATUS_EXPORTS'
run_one_overlay_red 'Task 6 v20 overlay retarget and blocked descriptor store primitives use dependency source account' 'TASK6V20_RED_003_RETARGET_DESCRIPTOR'
run_one_overlay_red 'Task 6 v20 overlay startup renews after recovery before ready and publishes after commit' 'TASK6V20_RED_004_STARTUP_RENEW'
run_one_overlay_red 'Task 6 v20 overlay advance due keeps live order and enqueues continuation at sixty one' 'TASK6V20_RED_005_DUE61_ORDER'
run_one_overlay_red 'Task 6 v20 overlay maintenance cutover validates guards and release precedence' 'TASK6V20_RED_006_CUTOVER_RELEASE'
run_one_overlay_red 'Task 6 v20 overlay scheduler cutover CLI grammar lifecycle and exact exports' 'TASK6V20_RED_007_CLI'
grep -c '^### TASK6_V20_OVERLAY_RED_CASE_START$' /tmp/task6-v20-red-evidence.tap | grep -E '^7$'
grep -c '^### TASK6_V20_OVERLAY_RED_CASE_END$' /tmp/task6-v20-red-evidence.tap | grep -E '^7$'
sha256sum /tmp/task6-v20-red-evidence.tap > /tmp/task6-v20-red-evidence.tap.sha256
```

## TDD implementation waves

### Wave 1: Skeletons, helper prerequisites, and parent import
...
- [ ] Add minimal module skeletons with exact export keys.
- [ ] Append helper prerequisite slice exactly between V20 helper markers.
- [ ] Append parent73 slice exactly between V20 parent markers.
- [ ] Run parent-only registration check:

```bash
node --test --test-isolation=none --test-reporter=tap --test-name-pattern '^$' tools/test-scheduler.js > /tmp/task6-v20-parent-registration.tap
node - <<'NODE' /tmp/task6-v20-parent-registration.tap
const fs = require('fs');
const tap = fs.readFileSync(process.argv[2], 'utf8');
const names = Array.from(tap.matchAll(/^# Subtest: (.+)$/gm)).map((m) => m[1]);
const parent = fs.readFileSync('/tmp/task6-v20-parent73-names.txt', 'utf8').trim().split('\n');
const baseline = fs.readFileSync('/tmp/task6-v20-baseline-names.txt', 'utf8').trim().split('\n');
if (names.length !== baseline.length + parent.length) throw new Error('TASK6_V20_PARENT_REGISTRATION_COUNT_' + names.length);
if (names.slice(0, baseline.length).join('\n') !== baseline.join('\n')) throw new Error('TASK6_V20_BASELINE_REGISTRATION_CHANGED');
if (names.slice(baseline.length).join('\n') !== parent.join('\n')) throw new Error('TASK6_V20_PARENT_REGISTRATION_ORDER_CHANGED');
for (const name of parent) if (!names.includes(name)) throw new Error('TASK6_V20_PARENT_NOT_REGISTERED ' + name);
if (parent.length !== 73) throw new Error('TASK6_V20_PARENT_EXPECTED_COUNT');
NODE
```

- [ ] After production implementation exists, run at least one parent fixture construction target and require no `ReferenceError`:
...
node --test --test-isolation=none --test-reporter=tap \
  ...
  tools/test-scheduler.js > /tmp/task6-v20-fixture-smoke.tap
if rg -n 'ReferenceError|is not defined|MISSING_PREREQUISITE' /tmp/task6-v20-fixture-smoke.tap; then exit 1; fi
```
...
```bash
node --test --test-isolation=none --test-reporter=tap \
  --test-name-pattern '^(Task 6 v20 overlay raw quarantine store primitive preserves attempt without parsing invalid payload|Task 6 v20 overlay retarget and blocked descriptor store primitives use dependency source account)$' \
  tools/test-scheduler.js
```
...
- [ ] Implement startup recovery with list/recover/integrity/renew/rebase/final fence/publish order.
...
```bash
node --test --test-isolation=none --test-reporter=tap \
  --test-name-pattern '^(Task 6 v20 overlay status cache private seams and exact scheduler exports|Task 6 v20 overlay startup renews after recovery before ready and publishes after commit|Task 6 v20 overlay advance due keeps live order and enqueues continuation at sixty one)$' \
  tools/test-scheduler.js
```
...
- [ ] Implement `runMaintenanceCutover({kho, clock, ownerId})`.
- [ ] Implement CLI `parseArgs`, `runCli`, and `main`.
- [ ] Run overlay F/G:

```bash
node --test --test-isolation=none --test-reporter=tap \
  --test-name-pattern '^(Task 6 v20 overlay maintenance cutover validates guards and release precedence|Task 6 v20 overlay scheduler cutover CLI grammar lifecycle and exact exports)$' \
  tools/test-scheduler.js
```

## Stage-A and Stage-B validation

Create report draft before post-inventory:
...
```bash
cat > /tmp/task6-v20-six-meta.sh <<'SH'
make_six_meta() {
  python3 - <<'PY'
import hashlib, os, stat, subprocess
...
SH
sha256sum /tmp/task6-v20-six-meta.sh > /tmp/task6-v20-six-meta.sh.sha256
. /tmp/task6-v20-six-meta.sh
make_six_meta > /tmp/task6-v20-six-meta.sealed.tsv
sha256sum /tmp/task6-v20-six-meta.sealed.tsv > /tmp/task6-v20-six-meta.sealed.tsv.sha256
```

Create Stage-A script:

```bash
cat > /tmp/task6-v20-stage-a.sh <<'SH'
set -euo pipefail
report='docs/superpowers/reports/2026-08-25-durable-scheduler-task6-writer-lifecycle-implementation-report.md'
sha256sum -c /tmp/task6-v20-stage-a.sh.sha256
sha256sum -c /tmp/task6-v20-approved-plan.sha256
sha256sum -c /tmp/task6-v20-authorities.sha256
sha256sum -c /tmp/task6-v20-inventory.sh.sha256
sha256sum -c /tmp/task6-v20-pre-inventory.tsv.sha256
sha256sum -c /tmp/task6-v20-tap-normalize.sh.sha256
sha256sum -c /tmp/task6-v20-baseline-names.txt.sha256
sha256sum -c /tmp/task6-v20-prefix.tsv.sha256
sha256sum -c /tmp/task6-v20-expected-helper-slice.js.sha256
sha256sum -c /tmp/task6-v20-helper-parts.sha256
sha256sum -c /tmp/task6-v20-helper-free-identifiers.txt.sha256
sha256sum -c /tmp/task6-v20-parent73-names.txt.sha256
sha256sum -c /tmp/task6-v20-parent-blocks.sha256
sha256sum -c /tmp/task6-v20-expected-parent-slice.js.sha256
sha256sum -c /tmp/task6-v20-expected-overlay.js.sha256
sha256sum -c /tmp/task6-v20-overlay7-names.txt.sha256
sha256sum -c /tmp/task6-v20-red-evidence.tap.sha256
sha256sum -c /tmp/task6-v20-six-meta.sh.sha256
sha256sum -c /tmp/task6-v20-six-meta.sealed.tsv.sha256

node - <<'NODE' > /tmp/task6-v20-current-helper-slice.js
const fs = require('fs');
const text = fs.readFileSync('tools/test-scheduler.js', 'utf8');
const start = '// TASK6_V20_HELPERS_START\n';
const end = '// TASK6_V20_HELPERS_END\n';
const a = text.indexOf(start);
const b = text.indexOf(end);
if (a < 0 || b < 0 || b <= a) throw new Error('TASK6_V20_HELPER_MARKERS_INVALID');
process.stdout.write(text.slice(a + start.length, b));
NODE
cmp -s /tmp/task6-v20-expected-helper-slice.js /tmp/task6-v20-current-helper-slice.js
if rg -n '^[[:space:]]*test[[:space:]]*\\(' /tmp/task6-v20-current-helper-slice.js; then exit 1; fi

node - <<'NODE' > /tmp/task6-v20-current-parent-slice.js
const fs = require('fs');
const text = fs.readFileSync('tools/test-scheduler.js', 'utf8');
const start = '// TASK6_V20_PARENT_TESTS_START\n';
const end = '// TASK6_V20_PARENT_TESTS_END\n';
const a = text.indexOf(start);
const b = text.indexOf(end);
if (a < 0 || b < 0 || b <= a) throw new Error('TASK6_V20_PARENT_MARKERS_INVALID');
process.stdout.write(text.slice(a + start.length, b));
NODE
cmp -s /tmp/task6-v20-expected-parent-slice.js /tmp/task6-v20-current-parent-slice.js

node - <<'NODE' > /tmp/task6-v20-current-overlay.js
const fs = require('fs');
const text = fs.readFileSync('tools/test-scheduler.js', 'utf8');
const start = '// TASK6_V20_OVERLAY_TESTS_START\n';
const end = '// TASK6_V20_OVERLAY_TESTS_END\n';
const a = text.indexOf(start);
const b = text.indexOf(end);
if (a < 0 || b < 0 || b <= a) throw new Error('TASK6_V20_OVERLAY_MARKERS_INVALID');
process.stdout.write(text.slice(a + start.length, b));
NODE
cmp -s /tmp/task6-v20-expected-overlay.js /tmp/task6-v20-current-overlay.js

node - <<'NODE'
const fs = require('fs'), crypto = require('crypto');
const [lenText, sha] = fs.readFileSync('/tmp/task6-v20-prefix.tsv', 'utf8').trim().split('\t');
const data = fs.readFileSync('tools/test-scheduler.js').subarray(0, Number(lenText));
const actual = crypto.createHash('sha256').update(data).digest('hex');
if (actual !== sha) throw new Error('TASK6_V20_PREFIX_CHANGED');
NODE

. /tmp/task6-v20-six-meta.sh
make_six_meta > /tmp/task6-v20-six-meta.stage-a-before.tsv
diff -u /tmp/task6-v20-six-meta.sealed.tsv /tmp/task6-v20-six-meta.stage-a-before.tsv
...
node --test --test-isolation=none --test-reporter=tap tools/test-scheduler.js > /tmp/task6-v20-stage-a.tap
node --throw-deprecation --test --test-isolation=none --test-reporter=tap tools/test-scheduler.js > /tmp/task6-v20-stage-a-throw.tap
for f in /tmp/task6-v20-stage-a.tap /tmp/task6-v20-stage-a-throw.tap; do
  grep -E '^# tests 243$' "$f"
  grep -E '^# pass 242$' "$f"
  grep -E '^# fail 0$' "$f"
  grep -E '^# skipped 1$' "$f"
...
. /tmp/task6-v20-tap-normalize.sh
normalize_tap_for_hash /tmp/task6-v20-stage-a.tap > /tmp/task6-v20-stage-a.normalized.tap
normalize_tap_for_hash /tmp/task6-v20-stage-a-throw.tap > /tmp/task6-v20-stage-a-throw.normalized.tap
sha256sum /tmp/task6-v20-stage-a.normalized.tap > /tmp/task6-v20-stage-a.normalized.tap.sha256
sha256sum /tmp/task6-v20-stage-a-throw.normalized.tap > /tmp/task6-v20-stage-a-throw.normalized.tap.sha256

node - <<'NODE' /tmp/task6-v20-stage-a.tap
const fs = require('fs');
const tap = fs.readFileSync(process.argv[2], 'utf8');
const names = Array.from(tap.matchAll(/^# Subtest: (.+)$/gm)).map((m) => m[1]);
const baseline = fs.readFileSync('/tmp/task6-v20-baseline-names.txt', 'utf8').trim().split('\n');
const parent = fs.readFileSync('/tmp/task6-v20-parent73-names.txt', 'utf8').trim().split('\n');
const overlay = fs.readFileSync('/tmp/task6-v20-overlay7-names.txt', 'utf8').trim().split('\n');
if (names.length !== 243) throw new Error('TASK6_V20_NAME_COUNT_' + names.length);
if (new Set(overlay).size !== 7) throw new Error('TASK6_V20_OVERLAY_DUPLICATE');
const expected = [...baseline, ...parent, ...overlay];
if (names.join('\n') !== expected.join('\n')) throw new Error('TASK6_V20_NAME_ORDER_CHANGED');
NODE

make_six_meta > /tmp/task6-v20-six-meta.stage-a-after.tsv
diff -u /tmp/task6-v20-six-meta.stage-a-before.tsv /tmp/task6-v20-six-meta.stage-a-after.tsv
...
. /tmp/task6-v20-inventory.sh
repo_inventory > /tmp/task6-v20-post-inventory.tsv
python3 - <<'PY'
from pathlib import Path
pre = {line.split('\t',1)[0]: line for line in Path('/tmp/task6-v20-pre-inventory.tsv').read_text().splitlines()}
post = {line.split('\t',1)[0]: line for line in Path('/tmp/task6-v20-post-inventory.tsv').read_text().splitlines()}
allowed = {
...
for key in sorted(set(pre) | set(post)):
    if pre.get(key) != post.get(key) and key not in allowed:
        diffs.append(key)
if diffs:
    raise SystemExit('TASK6_V20_SCOPE_DIFF ' + ','.join(diffs))
PY
sha256sum /tmp/task6-v20-post-inventory.tsv > /tmp/task6-v20-post-inventory.tsv.sha256
...
grep -F "plan_v20_sha=$(cut -d' ' -f1 /tmp/task6-v20-approved-plan.sha256)" "$report"
...
grep -F "pre_inventory_sha=$(cut -d' ' -f1 /tmp/task6-v20-pre-inventory.tsv.sha256)" "$report"
grep -F "post_inventory_sha=$(cut -d' ' -f1 /tmp/task6-v20-post-inventory.tsv.sha256)" "$report"
grep -F "helper_slice_sha=$(cut -d' ' -f1 /tmp/task6-v20-expected-helper-slice.js.sha256)" "$report"
grep -F "helper_free_identifiers_sha=$(cut -d' ' -f1 /tmp/task6-v20-helper-free-identifiers.txt.sha256)" "$report"
grep -F "prefix_len=$(cut -f1 /tmp/task6-v20-prefix.tsv)" "$report"
grep -F "prefix_sha=$(cut -f2 /tmp/task6-v20-prefix.tsv)" "$report"
grep -F "baseline_names_sha=$(cut -d' ' -f1 /tmp/task6-v20-baseline-names.txt.sha256)" "$report"
grep -F "parent73_names_sha=$(cut -d' ' -f1 /tmp/task6-v20-parent73-names.txt.sha256)" "$report"
grep -F "overlay7_names_sha=$(cut -d' ' -f1 /tmp/task6-v20-overlay7-names.txt.sha256)" "$report"
grep -F "parent_slice_sha=$(cut -d' ' -f1 /tmp/task6-v20-expected-parent-slice.js.sha256)" "$report"
grep -F "overlay_slice_sha=$(cut -d' ' -f1 /tmp/task6-v20-expected-overlay.js.sha256)" "$report"
grep -F "overlay_red_evidence_sha=$(cut -d' ' -f1 /tmp/task6-v20-red-evidence.tap.sha256)" "$report"
grep -F "stage_a_script_sha=$(cut -d' ' -f1 /tmp/task6-v20-stage-a.sh.sha256)" "$report"
grep -F "stage_a_normalized_tap_sha=$(cut -d' ' -f1 /tmp/task6-v20-stage-a.normalized.tap.sha256)" "$report"
grep -F "stage_a_throw_normalized_tap_sha=$(cut -d' ' -f1 /tmp/task6-v20-stage-a-throw.normalized.tap.sha256)" "$report"
...
while IFS= read -r row; do grep -F "six_meta=$row" "$report"; done < /tmp/task6-v20-six-meta.sealed.tsv
SH
chmod +x /tmp/task6-v20-stage-a.sh
sha256sum /tmp/task6-v20-stage-a.sh > /tmp/task6-v20-stage-a.sh.sha256
```

Stage-A execution:

```bash
/tmp/task6-v20-stage-a.sh
```

Review handoff:

- Launch two distinct fresh implementation reviewers using model `gpt-5.6-sol` with effort `high`.
- Review payload includes exact six rows from `/tmp/task6-v20-six-meta.sealed.tsv`, V20 approved plan hash, parent/Task5 hashes, helper/parent/overlay slice hashes, overlay RED evidence hash, Stage-A TAP counts, and normalized hashes.
- Any six-file, plan, helper-slice, parent-slice, overlay-slice, evidence, or Stage-A artifact mismatch invalidates those reviews and requires new Stage-A plus two fresh reviews.
...
- [ ] V20 count is parent runtime `73` plus overlay `7`, final `243/242/0/1`.
- [ ] V18 replay and partial-wrapper overlays remain absent.
- [ ] Helper-only prerequisite slice exists before parent73 and contains no `test(` registrations.
- [ ] Helper source ranges are exact: `6411-6470`, `7601-7609` plus V20 imports, `7677-7744`, `7748-7850`, `7855-7904`, raw `8134-8144`.
- [ ] Parent block 3 starts at `12489`.
- [ ] Static helper free-identifier allowlist is sealed and Stage-A checks helper/current slice bytes.
- [ ] RED harness is `set -euo pipefail`, fail-closed, rejects ReferenceError/missing prerequisite/fixture setup failures, and requires each sentinel.
- [ ] Overlay A uses invalid JSON bytes and only claims direct Store raw quarantine behavior.
- [ ] Overlay B covers deep clone metrics, DB-close cache, exact index exports, and non-enumerable private seams `_datSignalHandlerInstalled`, `_beginStop`, `_waitForStopFinalization`, `_datDatabaseClosing`.
- [ ] Overlay C uses the dependency source account, `targetS<=nowS`, retargets same-key wake, and validates frozen blocked descriptor.
- [ ] Overlay D spies startup recovery list methods, renew, resume, final fence, asserts renew before ready and timers after ready commit.
- [ ] Overlay E proves live `dqDenHan` order `[3,1,2]`, 61 boundary partial, closure skipped, and continuation active.
- [ ] Overlay F proves success release same token, already durable guard, nonempty guard, primary-over-release precedence, and release-only propagation.
- [ ] Overlay G proves duplicate/unknown/missing/extra grammar, safe errors, primary-vs-close precedence, close-only behavior, and restores `process.exitCode`.
- [ ] Report file and reports directory are excluded from normalized content inventory/diff and separately validated for real type/mode/content fields.
- [ ] Stage-A creates and validates every referenced artifact from scratch, runs fresh normal and throw-deprecation tests, compares helper/parent/overlay slices, validates scope/inventory, and parses report fields.
